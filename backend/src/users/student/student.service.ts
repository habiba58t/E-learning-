...
// async getnumberOfEasyLevel(course_code: string): Promise<number> {
//   // Validate course_code
//   if (!mongoose.Types.ObjectId.isValid(course_code)) {
//     throw new Error(`Invalid course_code: ${course_code}`);
//   }

//   let courseObjectId: mongoose.Types.ObjectId;
//   try {
//     courseObjectId = new mongoose.Types.ObjectId(course_code);
//   } catch (error) {
//     console.error(`Error converting course_code to ObjectId: ${error.message}`);
//     throw new Error(`Failed to process course_code: ${course_code}`);
//   }

//   const students = await this.progressService.findAllStudentsEnrolled(course_code);

//   const easyCount = (await Promise.all(
//     students.map(async (student) => {
//       const level = await this.getStudentLevel(student, courseObjectId);
//       return level === 'easy' ? 1 : 0;
//     })
