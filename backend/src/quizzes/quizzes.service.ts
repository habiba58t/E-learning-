...
async getQuizzesByInstructorUsername(username: string): Promise<Quiz[]> {
  return await this.quizModel.find({ created_by: username }).exec();  // Assuming `created_by` is a string (username)
}


// async getQuizzesByInstructorUsername(username: string): Promise<any[]> {
//   // Fetch quizzes where created_by matches the instructor's username
//   const quizzes = await this.quizModel
//     .find({ created_by: username }) // Match by created_by field
//     .exec();

//   return quizzes.map(quiz => ({
//     quizId: quiz._id,
//     noOfQuestions: quiz.no_of_questions,
//     typesOfQuestions: quiz.types_of_questions,
//     isOutdated: quiz.isOutdated,
//   }));
// }
