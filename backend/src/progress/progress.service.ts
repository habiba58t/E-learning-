...
export class ProgressService {
  constructor(
    ...
    //@InjectModel(Quiz.name) private readonly quizModel: Model<Quiz>,
     ...
     private coursesService: CoursesService //private usersService: UsersService, private quizService:QuizzesService
  ) {}
    ...
    async findAllByCourse(course_code: string): Promise<Progress[]> {
  ...
  async findbyCourseCode(course_code: string):Promise<Progress>{ //for a student finding a progress in a course 
    ...
    async update(
      ...
      async findNumberOfStudentsEnrolled(course_code: string): Promise<number> {
        const students = await this.findAllByCourse(course_code);
        return students.length;
      }
      async findAllStudentsEnrolled(course_code: string): Promise<string[]> {
        const students = await this.findAllByCourse(course_code);
